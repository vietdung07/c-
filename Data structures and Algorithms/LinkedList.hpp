...
template <typename T1>
void LinkedList<T1>::Mergesort()
{
    if (this->head == this->tail)
        return; // base case
    // temp is the center node;
    NODE<T1> *mid = this->head;
    int i = 1;
    for (; i <= (this->size) / 2 - 1; i++)
    {
        mid = mid->nxt;
    }
    // move the first halft part resources to list1
    LinkedList<T1> list1;
    list1.head = this->head;
    list1.tail = mid;
    list1.size = i;
    // move the second halft part resources to list2
    LinkedList<T1> list2;
    list2.head = mid->nxt;
    list2.tail = this->tail;
    list2.size = this->size - i;
    // detach the origin resources
    this->head = nullptr;
    this->tail = nullptr;
    // separate the origin list
    list1.tail->nxt = nullptr;
    list2.head->pre = nullptr;
    // sort the two parts;
    list1.Mergesort();
    list2.Mergesort();
    // set tracking nodes
    NODE<T1> *pos1 = list1.head;
    NODE<T1> *pos2 = list2.head;
    // check special case
    //  return resources to the origin list;
    list1.tail->nxt = list2.head;
    ...
    list2.size = 0;
    // merge......
    if (pos2->pre->data <= pos2->data)
        return;
    while (pos1 != pos2 && pos2 != nullptr)
    {
        while (pos1->data < pos2->data && pos1 != pos2)
        {
            pos1 = pos1->nxt;
        }
        if (pos1 == pos2)
